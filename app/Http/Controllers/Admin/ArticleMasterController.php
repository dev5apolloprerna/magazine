...
class ArticleMasterController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'article_image' => 'required|file|mimes:jpg,jpeg,png,webp|max:2048',
        ...
        ArticleMaster::create([
            ...
            'article_pdf'   => $pdfPath,
            'isPaid'       => $request->filled('isPaid') ? (int)$request->isPaid : 1,
    ...
    {
        $article = ArticleMaster::where('article_id', (int)$id)->where('isDelete', 0)->firstOrFail();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->filled('iStatus')) $article->iStatus = (int)$request->iStatus;
        if ($request->filled('isPaid')) $article->isPaid = (int)$request->isPaid;
    ...
    {
        $absFolder = env('MAGAZINE_IMAGE_DIR',$folder);
        if (!is_dir($absFolder)) {
